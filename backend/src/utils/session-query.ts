...
export async function bulkUpsertSessions(events: TrackedEvent[]): Promise<void> {
...
export async function findAllSessions(filters: SessionFilters = {}): Promise<SessionSummary[]> {
  const query: any = {};
...
function escapeRegExp(str: string): string {
  ...
  // ponytail: query pre-computed deviceType field
...
export async function findExportSessions(filters: { startDate?: string; endDate?: string; page?: number; limit?: number }): Promise<{ total: number; sessions: ISession[] }> {
  const query: any = {};
  if (filters.startDate || filters.endDate) {
    query.lastSeen = {};
    if (filters.startDate) query.lastSeen.$gte = new Date(filters.startDate);
